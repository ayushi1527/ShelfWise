...
export default function Consumer() {
  ...
  const consumerFeatures = [
    { icon: "🎯", text: "Personalized Recommendations", color: "border-blue-500" },
    { icon: "🤖", text: "AI Shopping Assistant", color: "border-blue-500" },
    { icon: "📱", text: "Smart QR Education", color: "border-blue-500" },
    { icon: "♻️", text: "Sustainability Tracker", color: "border-blue-500" },
  ...
  const activities = [
    { id: 1, task: "AI Assistant Chat", status: "Completed", date: "Today, 2:45 PM" },
    { id: 2, task: "QR Product Scan", status: "Viewed", date: "Yesterday, 11:20 AM" },
    { id: 3, task: "Flash Deal Claimed", status: "Success", date: "Oct 24, 2023" },
  ];
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <aside 
        ...
        ${isSidebarOpen ? 'translate-x-0' : '-translate-x-72'} lg:translate-x-0`}
      >
        <div className="p-6 text-2xl font-bold border-b border-blue-500 flex items-center gap-3">
          <div className="h-10 w-10 bg-white text-blue-600 rounded-lg flex items-center justify-center font-black">W</div>
          ShelfWise
        </div>
        
        <nav className="p-4 flex flex-col h-[calc(100%-80px)] justify-between">
          <ul className="space-y-2">
            <p className="text-xs font-semibold text-blue-200 uppercase tracking-wider mb-2 px-2">Main Menu</p>
            {consumerFeatures.map((f, i) => (
              ...
              >
                <span className="text-xl group-hover:scale-120 transition-transform">{f.icon}</span>
          ...
          <div className="p-4 bg-blue-700 rounded-2xl mb-4">
            ...
            <Link to="/login" className="text-xs underline mt-2 block opacity-70 hover:opacity-100">Logout</Link>
          </div>
        </nav>
      </aside>
      ...
      <div className="flex-1 lg:ml-72 flex flex-col">
        ...
        <nav className="bg-white border-b border-gray-200 p-4 sticky top-0 flex justify-between items-center z-40">
          <div className="flex items-center gap-4">
            ...
            <h2 className="text-xl font-bold text-gray-800 hidden sm:block">Consumer Dashboard</h2>
          </div>
        ...
        <main className="p-6 lg:p-10 max-w-7xl mx-auto w-full">
          ...
          <div className="mb-10">
            ...
            <p className="text-gray-500 mt-1">Here is what’s happening with your smart shopping experience.</p>
          </div>
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {activities.map((item) => (
                      <tr key={item.id} className="hover:bg-gray-50/50 transition-colors">
                        <td className="p-5 font-medium text-gray-700">{item.task}</td>
                        <td className="p-5">
                          <span className={`px-3 py-1 rounded-full text-xs font-bold 
                            ${item.status === 'Completed' ? 'bg-green-100 text-green-700' : 
                              item.status === 'Success' ? 'bg-blue-100 text-blue-700' : 'bg-yellow-100 text-yellow-700'}`}>
                            {item.status}
                          </span>
                        </td>
          ...
          {/* FEATURE GRID */}
          <section>
            <h3 className="text-xl font-bold text-gray-800 mb-6">Explore AI Features</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
              {consumerFeatures.map((feature, index) => (
                <FeatureCard 
                  ...
                  onClick={() => alert(`Starting ${feature.text}...`)}
                />
              ))}
            </div>
          </section>
